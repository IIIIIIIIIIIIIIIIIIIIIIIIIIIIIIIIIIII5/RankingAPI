...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName("set").setDescription("Set the Roblox group ID for this server.").addIntegerOption(opt => opt.setName("groupid").setDescription("The Roblox group ID to link to this server.").setRequired(true)))
        .addSubcommand(sub => sub.setName("remove").setDescription("Request to remove the configured Roblox group from this server.")),
    ...
    async execute(interaction, verifications, PendingApprovals) {
        ...
        if (sub === "set") {
            ...
            if (existing && existing.GroupId) return interaction.reply({ content: "This server already has a configured group. Use `/config remove` to request removal first.", ephemeral: true });
        ...
        if (sub === "remove") {
            if (!existing || !existing.GroupId) return interaction.reply({ content: "No group is currently configured for this server.", ephemeral: true });
            ...
            await interaction.reply({ content: `A removal request for group ID ${GroupId} has been sent for approval.`, ephemeral: true });
        }
    }
};
